...
std::cout << "tras ordenar, el primero está a " << stair.vPlanes[0].centroid2f.y << std::endl;
    ...
    if (stair.vPlanes.size() > 1) { // Si hay más de un posible peldaño
        for (size_t Q=0; Q<(stair.vPlanes.size()); Q++) { // Se recorren todos ellos
...
std::cout << "creo nuevo suelo" << std::endl;
                    Plane temp_plane(stair.vPlanes[Q].cloud, stair.vPlanes[Q].coeffs);
...
std::cout << "Hay al menos un nivel, a ver qué pasa con este" << std::endl;
                ...
                for (int P = max_level; P >= 0; P--) {
                    ...
                    is_near = (neighbour_search(stair.vLevels[size_t(P)].cloud,stair.vPlanes[Q].centroid,0.50f) > 5);
...
std::cout << "está cerca de alguno anterior!" << std::endl;
                    if (fabs(fabs(stair.vPlanes[Q].centroid2f.y)-fabs(stair.vLevels[size_t(max_level)].centroid2f.y)) < 0.07f) { // Si la diferencia entre niveles es mínima, se trata del mismo nivel
